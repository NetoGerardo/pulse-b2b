        ...
        <div class="card card-success">
          ...
          <form @submit.prevent enctype="multipart/form-data">
            <div class="card-footer">
              ...
              <button @click="reenviarProposta()" class="btn btn-flat btn-success">Enviar para análise</button>
            </div>
          </form>
        </div>
      </div>
        ...
        <div class="card card-primary">
          ...
          <div class="card-body" style="padding: 10px 10px 10px 10px">
            ...
            <div class="form-group">
              ...
              <form enctype="multipart/form-data">
                <label>Identidade</label>
                <input v-show="vida.enviar_nova_identidade" class="form-control" type="file"
              ...
              <button v-if="!vida.enviar_nova_identidade" @click="vida.enviar_nova_identidade = true"
                class="btn btn-outline-success">Enviar
                novo</button>

              <button v-if="vida.enviar_nova_identidade" style="float: left;"
                @click="vida.enviar_nova_identidade = false" class="btn
                  btn-outline-danger">Cancelar</button>

              <button v-if="vida.enviar_nova_identidade" style="float: right;"
                @click="capturarArquivosDependentes(indice)" class="btn
            ...
            <div class="form-group">
              ...
              <form enctype="multipart/form-data">
                <label>CPF</label>
                <input v-show="vida.enviar_novo_cpf" class="form-control" type="file"
              ...
              <button v-if="vida.enviar_novo_cpf" style="float: right;" @click="capturarArquivosDependentes(indice)"
                class="btn
                  btn-outline-success">Upload</button>
            </div>
...
export default {
  ...
  methods: {
    ...
    capturarArquivosEmpregado(indice) {
      ...
      let arquivo = arquivosEmpregados[indice].key[0].files[0];

      if (arquivo) {
        if (allowedTypes.includes(arquivo.type)) {
          this.empregados[indice].arquivoComprovanteResidencia = arquivo;
        } else {
          error = true;
        ...
        error = true;
        message = "Envie o arquivo do empregado " + (indice + 1) + " !";
      }

      if (error) {
        this.showErrorMessageWithButton("Ops...", message);
      } else {
        this.updateEmpregados(indice);
      }
    },

    capturarArquivosDependentes(indice) {
      ...
      if (this.vidas[indice].enviar_nova_identidade) {
        ...
        //IDENTIDADES DEPENDENTES
        let arquivoIdentidade = identidadeDependentes[indice].key[0].files[0];

        if (arquivoIdentidade) {
          if (allowedTypes.includes(arquivoIdentidade.type)) {
            this.vidas[indice].arquivoIdentidade = arquivoIdentidade;
          } else {
            error = true;
            message = "Apenas imagens e PDF's são permitidos!";
          }
          ...
          error = true;
          message = "Envie a identidade do familiar " + (indice + 1) + " !";
        }
      }

      if (this.vidas[indice].enviar_novo_cpf) {
        //CPF'S DEPENDENTES
        let cpfDependentes = []
        for (let [key, value] of Object.entries(this.$refs)) {
        ...
        console.log(cpfDependentes);

        //CPF'S DEPENDENTES
        let arquivoCPF = cpfDependentes[indice].key[0].files[0];

        if (arquivoCPF) {
          if (allowedTypes.includes(arquivoCPF.type)) {
            this.vidas[indice].arquivoCPF = arquivoCPF;
          } else {
            error = true;
            message = "Apenas imagens e PDF's são permitidos!";
          }
          ...
          error = true;
          message = "Envie o cpf do familiar " + (indice + 1) + " !";
        }
      }
      ...
      if (error) {
        ...
        this.updateDependentes(indice);
      }
    },

    async updateEmpregados(indice) {

      this.empregados[indice].comprovante_residencia = await this.sendFile(this.empregados[indice].arquivoComprovanteResidencia);
      ...
      axios
        .post(`/user/proposta/update-empregados`, data)
        .then((response) => {
          this.showSuccessMessage("Proposta atualizada!");
          this.empregados[indice].enviar_novo = false;
        })
        .catch((error) => {
    ...
    async updateDependentes(indice) {

      //CPF
      if (this.vidas[indice].enviar_novo_cpf) {
        this.vidas[indice].arquivo_cpf = await this.sendFile(this.vidas[indice].arquivoCPF);
      }

      //IDENTIDADE
      if (this.vidas[indice].enviar_nova_identidade) {
        this.vidas[indice].arquivo_identidade = await this.sendFile(this.vidas[indice].arquivoIdentidade);
      }
      ...
      axios
        .post(`/user/proposta/update-dependentes`, data)
        .then((response) => {
          this.showSuccessMessage("Proposta atualizada!");
          this.vidas[indice].enviar_novo_cpf = false;
          this.vidas[indice].enviar_nova_identidade = false;
        })
        .catch((error) => {
          this.showErrorMessageWithButton("Ops..", error);
          console.log(error);
