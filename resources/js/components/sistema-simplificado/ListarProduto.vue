<template>
    <div>
        <admin-cadastrar-produto :operadoras="operadoras" />
        <div class="card-body p-0">
            <table class="table">
                <thead>
                    <tr>
                        <th style="width: 25%">Nome</th>
                        <th style="width: 25%">Operadora</th>
                        <th style="width: 12%">Gerenciar</th>
                        <th style="width: 12%">Deletar</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="produto in produtos" :key="produto">
                        <td data-label="Nome">{{ produto.nome }}</td>
                        <td data-label="Operadora">{{ produto.operadora }}</td>

                        <td>
                            <button data-toggle="modal" data-target="#detalhes-modal" @click="show(produto)" type="button"
                        ...
                        <td>
                            <button @click="deletar(produto)" type="button" class="btn btn-danger btn-sm">
                                Deletar
                    ...
                    <div class="modal-content">
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control" id="nome" type="text" v-model="produto_selecionado.nome" />
                            </div>

                            <div class="form-group">
                                <label for="email">Operadora</label>
                                <select class="form-control" id="operadora" v-model="produto_selecionado.operadora_id">
                            ...
                            <button data-toggle="modal" data-target="#detalhes-modal" type="button"
                                class="btn btn-primary btn-sm">
                                Voltar
                            </button>
...
export default {
    ...
    methods: {
        ...
        show(administradora) {
            this.produto_selecionado = administradora;
        },
        ...
        deletar(administradora) {
            ...
            this.$swal
                .fire({
                    title: "<h3 style='color:#616060'>Deseja deletar a produto<b><br/>" + this.produto_selecionado.nome + "</b>? </h3>",
                    icon: "warning",
                    padding: '1.5em',
                ...
                .then(result => {
                    if (result.value) {
                        ...
                        let data = {
                            id: this.produto_selecionado.id
                        }

                        axios
                            .post(`/admin/produto/delete`, data)
                            .then((response) => {

                                this.showSuccessMessage("Operadora deletada!");
        ...
        update() {
            ...
            let data = {
                id: this.produto_selecionado.id,
                nome: this.produto_selecionado.nome,
                operadora_id: this.produto_selecionado.operadora_id,
            };
            ...
            axios
                .post(`/admin/produto/update`, data)
                .then((response) => {

                    this.showSuccessMessage("Administradora atualizada!");
